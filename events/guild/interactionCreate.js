...
module.exports = async (client, interaction) => {
  ...
  if(interaction.isButton() && interaction.message.author.id == client.user.id && interaction.customId.includes("PREMIUM-")){
    ...
    if(!guild) return interaction.reply("❌ **I was kicked from the guild!**")
    ...
    if(requester){
      requester.send(`${interaction.customId == "PREMIUM-ACCEPT" ? `${client.emojiyes} **Your PREMIUM request for the server: \`${guild.name}\` got accepted!**` : `${client.emojino} **Your PREMIUM request for the server: \`${guild.name}\` got declined!**` }`).catch(()=>{});
    }
    if(interaction.customId == "PREMIUM-ACCEPT" && requester.id != guild.ownerId){
      guild.fetchOwner(owner => {
        owner.send(`${client.emojiyes} **Your PREMIUM request for the server: \`${guild.name}\` got accepted!**`).catch(()=>{});
      }).catch(()=>{});
    }
    if(interaction.customId == "PREMIUM-ACCEPT"){
      if(client.premium.get("global", "guilds").includes(guild.id)){
        interaction.update({embeds: [interaction.message.embeds[0].setTitle(`✅ Guild is already PREMIUM!`)], components: []})
      } else {
        client.premium.push("global", guild.id, "guilds");
        interaction.update({embeds: [interaction.message.embeds[0].setTitle(`${client.emojiyes} Accepted the Guild!`)], components: []})
      }
    } else {
      if(client.premium.get("global", "guilds").includes(guild.id)){
        interaction.update({embeds: [interaction.message.embeds[0].setTitle(`${client.emojiyes} Guild is already PREMIUM`)], components: []})
      } else {
        interaction.update({embeds: [interaction.message.embeds[0].setTitle(`${client.emojino} Denied the Guild!`)], components: []})
      }
    }
  }
