  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
            ...
            if (button.customId == "1") {
              ...
              await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
                ...
                .then(collected => {
                  ...
                  if (color.length != 7 && !color.includes("#")) return message.reply({embeds: [new Discord.MessageEmbed()
              ...
              await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
                ...
                .then(collected => {
                  ...
                  try {
                    ...
                    es = client.settings.get(message.guild.id, "embed")
                    return message.reply({embeds: [new Discord.MessageEmbed()
                      .setTitle(`${client.emojiyes} The new Embed Footer Text is:`.substr(0, 256))
                      .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
        ...
        collector.on('end', collected => {
          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `${client.emojiyes} **Selected the \`${collected.first().customId}\`. Button**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });
